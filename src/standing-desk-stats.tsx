...
export default function Command() {
  ...
  if (!stats) {
    return (
      <List>
        <List.EmptyView title="No Data" description="No tracking data available for this period" />
      </List>
    );
  }

  const progressBarLength = 20;
  const standingBars = Math.round((stats.standingPercentage / 100) * progressBarLength);
  ...
  return (
    ...
    >
      ...
      <List.Section title="Period Selection">
        ...
        <List.Item
          title="This Week"
          subtitle={period === "week" ? "Currently viewing" : "Switch to this week"}
          icon={period === "week" ? Icon.CheckCircle : Icon.Circle}
          actions={
            <ActionPanel>
              <Action title="View This Week" onAction={() => setPeriod("week")} />
            </ActionPanel>
          }
        />
        <List.Item
          title="This Month"
          subtitle={period === "month" ? "Currently viewing" : "Switch to this month"}
          icon={period === "month" ? Icon.CheckCircle : Icon.Circle}
          actions={
            <ActionPanel>
              <Action title="View This Month" onAction={() => setPeriod("month")} />
            </ActionPanel>
          }
        />
      </List.Section>
