...
export async function setState(state: DeskState, timestamp: number): Promise<void> {
  await Promise.all([
...
export function getSessionsForPeriod(sessions: Session[], period: "day" | "week" | "month"): Session[] {
  const now = new Date();
...
export function calculateStats(sessions: Session[]): Stats {
  ...
  const totalStanding = standingSessions.reduce((sum, s) => sum + s.duration, 0);
  const totalSitting = sittingSessions.reduce((sum, s) => sum + s.duration, 0);
  const totalTime = totalStanding + totalSitting;

  const standingPercentage = totalTime > 0 ? (totalStanding / totalTime) * 100 : 0;
  const sittingPercentage = totalTime > 0 ? (totalSitting / totalTime) * 100 : 0;
